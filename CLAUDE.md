...
2. **`collector.rs`** — Queries the live K8s API. Key design points:
   ...
   - Collection health rides along on every snapshot: `collection_duration_ms` (wall clock, measured from before the client handshake, since connecting is often what makes a pass slow) and `partial_failures`, the deduplicated sorted set of subsystems whose list call errored. Each is recorded at the existing `warn!` site through a `Degradations` recorder threaded into the fallible fetchers. The point is that a swallowed error and a genuinely empty result look identical downstream — no PDB rows reads exactly like no PDBs existing — and a recommendation derived from the second case is wrong rather than merely incomplete. A missing KEDA CRD stays out of the list: it is the normal state of most clusters, not a failure.
